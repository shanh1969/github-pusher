...
export default function Home() {
  ...
  const uploadFileToGitHub = async (filePath: string, content: string, isBase64: boolean = false) => {
    if (!selectedProject || !githubToken) return false;
    ...
    try {
      ...
      try {
        const getResponse = await fetch(
          `https://api.github.com/repos/${owner}/${repo}/contents/${fullPath}?ref=${selectedProject.branch}`,
          {
            headers: {
              'Authorization': `Bearer ${githubToken}`,
              'Accept': 'application/vnd.github.v3+json',
  ...
  // Handle adding new files (text or photos)
  const handleAddFiles = async () => {
    ...
    try {
      ...
      // Upload selected files (photos/binary)
      for (const file of selectedUploadFiles) {
        ...
        await uploadFileToGitHub(file.name, content, true);
        uploadedCount++;
      }

      // Upload text file if provided
      if (newFileName && newFileContent) {
        await uploadFileToGitHub(newFileName, newFileContent, false);
        uploadedCount++;
      }
      ...
      if (uploadedCount > 0) {
        ...
        // Refresh file list
        fetchFiles(selectedProject, currentPath);
      }
  ...
  return (
    <main className="min-h-screen flex flex-col">
                ...
                <div className="p-3 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/50">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="p-1.5 hover:bg-emerald-600/20 rounded-lg transition-colors text-emerald-400 hover:text-emerald-300"
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card w-full max-w-lg animate-fade-in max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              {/* Upload Photos/Files Section */}
              <div>
                <label className="block text-sm font-medium text-zinc-400 mb-2">
                  Upload Photos or Files
                </label>
                <div 
                  className="border-2 border-dashed border-zinc-700 rounded-lg p-6 text-center hover:border-emerald-500 transition-colors cursor-pointer"
                  onClick={() => addFileInputRef.current?.click()}
                >
                  <ImageIcon />
                  <p className="text-zinc-400 mt-2">Click to select files</p>
                  <p className="text-xs text-zinc-500 mt-1">Photos, images, or any files</p>
                </div>
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-2 bg-zinc-900 text-zinc-500">or create a new text file</span>
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="example.txt or folder/file.js"
                  className="input"
                />
              </div>
              ...
              {currentPath && (
                <p className="text-xs text-zinc-500">
                  Files will be added to: <span className="text-emerald-400">{currentPath}/</span>
                </p>
              )}
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button 
                ...
                disabled={uploadingFile || (selectedUploadFiles.length === 0 && (!newFileName || !newFileContent))}
                className="btn-primary flex items-center gap-2"
              >
                {uploadingFile ? (
                  <>
      ...
      {/* Hidden file input for Add File modal */}
      <input
        type="file"
        ref={addFileInputRef}
        onChange={handleFileSelect}
        accept="image/*,.pdf,.txt,.js,.jsx,.ts,.tsx,.html,.css,.json,.md,.py,.sql,.yaml,.yml,.xml,.svg"
        multiple
        className="hidden"
      />
    </main>
